...
- Shopify product preview: the native Admin Preview button first opens Shopify's `/products_preview` route. The Liquid bridge in `shopify/snippets/headless-product-preview.liquid` must be installed in the redirect theme to send a product-bound, five-minute HMAC to `/api/shopify-preview`. The endpoint validates the signature using `SHOPIFY_PRODUCT_PREVIEW_SECRET`, loads the product through the existing Admin client, and sets the signed `teavision_product_preview` cookie before redirecting to `/preview/products/<numeric-id>`. The standalone preview page uses the existing Admin product mapper and shared PDP with `purchasingDisabled`; keep it noindex, nofollow, and without product or breadcrumb JSON-LD. `/api/shopify-preview/disable` clears the session. Never log preview secrets, signatures, cookies, or product content.
- The Liquid bridge is a custom integration built from documented Shopify primitives, not a Shopify-provided headless preview API. Never authenticate a raw `preview_key`, trust a supplied product ID alone, or expose the shared secret in JavaScript or URLs. The old `?secret=` entry is retired. See `docs/testing/shopify-product-preview.md` for installation, rollout, and actual-button acceptance tests. Local tests do not prove the Shopify bridge is installed.
